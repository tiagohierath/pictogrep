...
if __name__ == "__main__":
    # Dark is the theme and light is the one you switch to, so dark sits on
    # bare :root and light waits behind the attribute the server stamps. This
    # is deliberately not a prefers-color-scheme query any more: the app picks
    # its own default rather than inheriting the system's.
    print(f"/* Generated by scripts/m3-palette.py from seed {SEED}. Do not hand-edit. */")
    print(":root {")
    emit("dark", 2)
    print("}\n")
    print(":root[data-theme=\"light\"] {")
    emit("light", 1)
    print("}")
